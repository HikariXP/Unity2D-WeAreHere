...
public class PauseMove : MonoBehaviour
{
    ...
    public void FixedUpdate()
    {
        ...
        if (GM.GetComponent<GM>().isMainLoaded && !GM.GetComponent<GM>().isMainStart && GM.GetComponent<GM>().isPause)
        {
            if (y > 0)
            {
        ...
        if (GM.GetComponent<GM>().isMainStart || !GM.GetComponent<GM>().isPause)
        {
            if (y < 2000)
            {
                y += speed * Time.deltaTime;
